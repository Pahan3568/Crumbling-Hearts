...
public class ModMenuIntegration implements ModMenuApi {
    ...
    private static class ConfigScreen extends Screen {
        ...
        protected ConfigScreen(Screen parent) {
            super(new TranslatableText("config.crumbling_hearts.title"));
            this.parent = parent;
        }
        ...
        protected void init() {
            ...
            this.normalColorField = new TextFieldWidget(
                    ...
                    200, 20,
                    new TranslatableText("config.crumbling_hearts.option.normal_heart_color")
            );
            ...
            this.extraColorField = new TextFieldWidget(
                    ...
                    this.width / 2 - 100, currentY,
                    200, 20,
                    new TranslatableText("config.crumbling_hearts.option.extra_heart_color")
            );
            ...
            this.contentHeight = fadeSpeedSlider.y + fadeSpeedSlider.getHeight();

            // Кнопки внизу экрана
            this.saveButton = new ButtonWidget(
        ...
        public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
            ...
            drawCenteredText(matrices, this.textRenderer,
                    new TranslatableText("config.crumbling_hearts.title"),
                    this.width / 2, 15,
                    0xFFFFFF);
            ...
            for (Element child : this.children()) {
                if (!(child == resetButton || child == saveButton)) {
                    if (child instanceof Drawable drawable) {
                        drawable.render(matrices, mouseX, mouseY - scrollOffset, delta);
                    }
                }
            }

            // Отрисовка цветовых превью
            renderColorPreview(matrices, normalColorField);
            renderColorPreview(matrices, extraColorField);

            // Рендерим лейблы
            renderLabels(matrices);
            ...
            // Рендерим скроллбар
            if (maxScroll > 0) {
                renderScrollbar(matrices, maxScroll, scrollableHeight);
            }
        ...
        private void renderLabels(MatrixStack matrices) {
            int currentY = START_Y - LABEL_OFFSET;

            drawLabel(matrices, "config.crumbling_hearts.option.normal_heart_color", currentY);
            currentY += SPACING;

            drawLabel(matrices, "config.crumbling_hearts.option.extra_heart_color", currentY);
            currentY += SPACING;

            drawLabel(matrices, "config.crumbling_hearts.option.particles", currentY);
            currentY += SPACING;

            drawLabel(matrices, "config.crumbling_hearts.option.gravity", currentY);
            currentY += SPACING;

            drawLabel(matrices, "config.crumbling_hearts.option.velocity", currentY);
            currentY += SPACING;

            drawLabel(matrices, "config.crumbling_hearts.option.fade_speed", currentY);
        }

        private void drawLabel(MatrixStack matrices, String translationKey, int y) {
        ...
        private boolean validateAndSave() {
            ...
            if (!HEX_PATTERN.matcher(normalColor).matches() || !HEX_PATTERN.matcher(extraColor).matches()) {
                errorMessage = new TranslatableText("config.crumbling_hearts.error.invalid_color");
                return false;
            }
        ...
        private void renderColorPreview(MatrixStack matrices, TextFieldWidget field) {
            int x = field.x + field.getWidth() + 10;
            int y = field.y;
            int color;
            ...
            fill(matrices, x - 1, y - 1, x + COLOR_PREVIEW_SIZE + 1, y + COLOR_PREVIEW_SIZE + 1, 0xFF000000);
            fill(matrices, x, y, x + COLOR_PREVIEW_SIZE, y + COLOR_PREVIEW_SIZE, color);
        }

        private void renderScrollbar(MatrixStack matrices, int maxScroll, int scrollableHeight) {
            ...
            fill(matrices, this.width - 10, HEADER_HEIGHT, this.width - 4, this.height - FOOTER_HEIGHT, 0x33FFFFFF); //отступы сверху и снизу
            fill(matrices, this.width - 9, scrollBarY, this.width - 5, scrollBarY + scrollBarHeight, 0xFFAAAAAA);
        }
    ...
    private static class CustomSliderWidget extends SliderWidget {
        ...
        public CustomSliderWidget(int x, int y, int width, int height, String key, float value, float min, float max) {
            super(x, y, width, height, new TranslatableText(""), (value - min) / (max - min));
            this.key = key;
            this.min = min;
            this.max = max;
        ...
        protected void updateMessage() {
            ...
            setMessage(new TranslatableText(translationKey).append(": " + valueStr));
        }
        @Override
